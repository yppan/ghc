   ...
   where
      go w c s
         ...
         = case readWord8OffAddr# addr c s of
            (# s', b #) -> go (w `or#` (extendWord8# b `uncheckedShiftL#` (c `uncheckedIShiftL#` 3#)))
                              (c +# 1#)
                              s'
...
wordFromAddrBE# n addr s0 = go 0## 0# s0
   where
      go w c s
         ...
         | True
         = case readWord8OffAddr# addr c s of
            (# s', b #) -> go ((w `uncheckedShiftL#` 8#) `or#` extendWord8# b)
                              (c +# 1#)
                              s'
      ...
      _ -> let
            go w c
               ...
               = case indexWord8Array# ba (word2Int# off +# c) of
                  b -> go (w `or#` (extendWord8# b `uncheckedShiftL#` (c `uncheckedIShiftL#` 3#)))
                          (c +# 1#)
           in go 0## 0#
...
wordFromByteArrayBE# n ba off = go 0## 0#
   where
      go w c
         ...
         | True
         = case indexWord8Array# ba (word2Int# off +# c) of
            b -> go ((w `uncheckedShiftL#` 8#) `or#` extendWord8# b) (c +# 1#)
